...
//load accidents table in an array
$arr_lat_accidents = array();
$arr_lng_accidents = array();
$p = 0;
$no_of_inter;
$fetch_accidents = mysqli_query($conn,"select latitude, longitude from features where feature = 'accident';");
if(($no_of_inter = mysqli_num_rows($fetch_accidents)) > 0)
{
	while($row = mysqli_fetch_assoc($fetch_accidents))
	{
		$arr_lat_accidents[$p] = $row["latitude"];
		$arr_lng_accidents[$p] = $row["longitude"];
		$p++;
	}
}
...
$p = 0;
for($k = 0;$k < sizeof($arr_lat_accidents);$k++)
{
	for($i = 0;$i < $num_routes;$i++)
	{
		$min = 1000;
		...
		{
				$distance = findDistance($arr_lat[$i][$j], $arr_lng[$i][$j], $arr_lat_accidents[$k], $arr_lng_accidents[$k]);
				if($distance < 15)
				{
					//echo '<br>route no:'.$i.' distance:'.$distance.' '.$arr_lat[$i][$j].' '.$arr_lng[$i][$j].' '.$arr_lat_accidents[$k].' '.$arr_lng_accidents[$k];
					if($distance < $min)
					{
						$min = $distance;
						$cur_lat = $arr_lat[$i][$j];
